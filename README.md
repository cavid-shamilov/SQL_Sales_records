...
-- Data Analysis & Business Key Problems & Answers```sql
...
```sql SELECT * 
FROM Salesrecords 
WHERE sale_date = '2022-09-19';```sql
...
```sqlSELECT * 
FROM Salesrecords 
WHERE total_sale > 1000;```sql


Q.6 Total number of transactions by gender in each category
```sqlSELECT category, gender, COUNT(transactions_id) 
FROM Salesrecords
GROUP BY category, gender;```sql
...
ORDER BY year(sale_date), MONTH(sale_date);```sql
